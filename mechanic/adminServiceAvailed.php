...
<?php
	...
	$dateTo=$_POST['dateto'];
	$sql="SELECT COUNT('serviceID'),`services`.serviceName,`problems`.problemName FROM `servicechecklist` 
		INNER JOIN `services` ON `servicechecklist`.serviceID=`services`.serviceID
		INNER JOIN `problems` ON `problems`.problemID=`services`.problem
		...
		<table id="items">
		 ...
		 ?>
		  <tr>
		      ...
		      <td colspan="1" class="total-line balance" style="text-align: center !important;">Total Services Availed: </td>
