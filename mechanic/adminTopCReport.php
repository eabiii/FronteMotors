...
<?php
	...
	$dateTo=$_POST['dateto'];
	$sql="SELECT COUNT('basicInspectionID'),
	`usertb`.`Firstname`,
	`usertb`.`Lastname`,
	`basic vehicle inspection`.`dateIn`,
	`basic vehicle inspection`.`dateOut`,
	`basic vehicle inspection`.`plateNo`,
	`vehicles`.`plateNo`,
	`vehicles`.`ownerID`,
	`usertb`.`UserID`  
	FROM `basic vehicle inspection` 
	...
	WHERE `basic vehicle inspection`.`dateIn` BETWEEN '".$dateFrom."' AND  '".$dateTo."' GROUP BY `basic vehicle inspection`.`plateNo` ORDER BY  COUNT('basicInspectionID') DESC LIMIT 10";
...
<body>
	...
	<div id="page-wrap">
		...
		<textarea id="header">Top Customer Report</textarea>
		...
		<table id="items">
		 ...
		 while($row=mysqli_fetch_assoc($result)){
		 	$plus +=$row["COUNT('basicInspectionID')"];
		 ?>
		 <tr>
		 ...
		 ?>
		  <tr>
		      ...
		      <td colspan="1" class="total-line balance" style="text-align: center !important;">Total People: </td>
